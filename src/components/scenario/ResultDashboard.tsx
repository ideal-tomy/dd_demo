    ...
    <section className="dd-dash">
      <div className="dd-dash__hero">
        ...
        <div className={`dd-kpi-row ${flashKpi ? "dd-flash" : ""}`}>
          ...
          <div className={`dd-kpi dd-kpi--wide-only ${ok ? "dd-kpi--ok" : "dd-kpi--ng"}`}>
            <p className="dd-kpi__label">目標達成率</p>
            <p className="dd-kpi__num">
              {reachPct}
              <span>%</span>
            </p>
            <p className="dd-kpi__diff">
              {ok ? "試算上は到達" : `目標 ${formatOkuFull(params.exitTarget)}`}
            </p>
          </div>
        </div>
      </div>

      <div className="dd-dash__analysis">
        <div className="dd-dash__col">
              ...
              {bars.map((b, i) => (
                <div key={b.l} className="dd-vbars__col">
                  <div
                    ...
                    style={{
                      height: `${Math.max(8, Math.round((heights[i] / maxH) * 100))}%`,
                      background: b.c,
                    }}
                  />
                </div>
      ...
      <div className="dd-dash__judgments">
